...
export const PhoneBackPreview: React.FC<{
  ...
  const officialImgUrl = useMemo(() => {
    if (modelId) {
      return getDeviceImage(modelId, brandId, colorName, customImageUrl);
    }
    ...
    return (
      <div className="h-6 flex items-center justify-center overflow-visible" style={{ flexShrink: 0 }}>
        <svg role="img" viewBox="0 0 48 48" width="20" height="20" aria-label="Google" style={{ flexShrink: 0 }}>
          ...
          <path fill="#4285F4" d="M24 20h22.6v8H24z"/>
          <path fill="#ffffff" d="M33.17 20H24v8h9.17v-8z"/>
        </svg>
      </div>
    );
  }
...
export const DeviceSelector: React.FC<DeviceSelectorProps> = ({
  ...
  const handleStorageSelect = (storage: number) => {
    ...
    if (selectedModel) {
      ...
      const baseVar = vars.find(v => v.storageGb === storage) || vars[0];
